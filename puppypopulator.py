...
shelter1 = Shelter(name="Oakland Animal Services", address="1101 29th Ave", city="Oakland", state="California",
                   zipcode="94601", website="oaklandanimalservices.org", maximum_capacity=20, current_occupancy=1)
...
shelter2 = Shelter(name="San Francisco SPCA Mission Adoption Center", address="250 Florida St", city="San Francisco",
                   state="California", zipcode="94103", website="sfspca.org", maximum_capacity=20, current_occupancy=5)
session.add(shelter2)

shelter3 = Shelter(name="Wonder Dog Rescue", address="2926 16th Street", city="San Francisco", state="California",
                   zipcode="94103", website="http://wonderdogrescue.org", maximum_capacity=20, current_occupancy=4)
session.add(shelter3)

shelter4 = Shelter(name="Humane Society of Alameda", address="PO Box 1571", city="Alameda", state="California",
                   zipcode="94501", website="hsalameda.org", maximum_capacity=20, current_occupancy=2)
...
shelter5 = Shelter(name="Palo Alto Humane Society", address="1149 Chestnut St.", city="Menlo Park", state="California",
                   zipcode="94025", website="paloaltohumane.org", maximum_capacity=20, current_occupancy=1)
...
# Function to check in puppies into a shelter
def PuppyCheckIn(puppy):
    shelter_id = puppy.shelter_id
    shelter = session.query(Shelter).filter_by(id = shelter_id).one()
    while shelter.current_occupancy >= shelter.maximum_capacity:
        shelter_id = input("Please provide a new shelter id: ")
        shelter = session.query(Shelter).filter_by(id = shelter_id).one()
    session.add(puppy)
    shelter.current_occupancy += 1
    session.commit()

# Function to adopt a puppy from a shelter
def AdoptPuppy(puppy, adopters):
    shelter_id = puppy.shelter_id
    shelter = session.query(Shelter).filter_by(id = shelter_id).one()
    shelter.current_occupancy -= 1
    puppy_profile = session.query(PuppyProfile).filter_by(puppy_id = puppy.id).one()
    puppy_profile.adopters = adopters
